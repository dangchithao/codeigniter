...
class Book extends CI_Controller
{
    ...
    public function danhcho()
    {
        ...
        if(isset($name)&& $name!="")
        {
            ...
            if(count($id)==0)
            {
                $data = array('danhcho'=>"Nội dung không khả dụng !", 'id'=>$id);
            }
            else
            {
    ...
    public function view_info_sach()
    {
        ...
        foreach($s->all as $ts)
        {
            ...
            $data = array('id'=>$ts->id, 'ten'=>$ts->tensach, 'gia'=>$ts->gia, 'giaban'=>$ts->giaban,
                ...
                'danhcho'=>$ts->danhcho, 'url_dc'=>$url_danhcho,'namxb'=>$ts->namxb, 'mota'=>substr($ts->mota, 0, 250).'...', 'mota2'=>$ts->mota, 'picture'=>$ts->picture);
        }
         ...
         */
        $total = array(); $ten = array(); $tacgia = array(); $theloai = array(); $nxb = array();
        $namxb = array(); $mota = array(); $nbook = array(); $picture = array(); $giaban = array();
        foreach($s->all as $ts)
        {
            $total[] = $ts->id;
            $ten[] = substr($ts->tensach, 0, 17).'...';
            $tacgia[] = $ts->tacgia;
            $nxb[] = $ts->nxb;
            $theloai[] = $ts->loaisach;
            $namxb[] = $ts->namxb;
        ...
        $data1 = array('total'=>$total, 'ten1'=>$ten, 'tacgia1'=>$tacgia, 'nxb1'=>$nxb, 'theloai1'=>$theloai,
                'namxb1'=>$namxb, 'giaban1'=>$giaban,'mota1'=>$mota, 'nbook'=>$nbook, 'pic'=>$picture);
